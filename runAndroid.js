...
function buildApk(args, gradlew) {
  try {
    ...
    const assemble = args.variant ? 'assemble' + args.variant[0].toUpperCase() + args.variant.slice(1) : 'assembleDebug';
    console.log('ASSEMBLE TASK', assemble);
    // using '-x lint' in order to ignore linting errors while building the apk
    child_process.execFileSync(gradlew, [assemble, '-x', 'lint'], {
      stdio: [process.stdin, process.stdout, process.stderr],
    });
  } catch (e) {
...
function tryInstallAppOnDevice(args, device) {
  try {
    ...
    const pathToApk = `${args.appFolder}/build/outputs/apk/${args.appFolder}-${variant}.apk`;
      console.log('INSTALLING FILE', pathToApk);
    const adbPath = getAdbPath();
    const adbArgs = ['-s', device, 'install', '-r', pathToApk];
    console.log(chalk.bold(
      `Installing the app on the device (cd android && adb -s ${device} install -r ${pathToApk}`
    ));
    child_process.execFileSync(adbPath, adbArgs, {
      stdio: [process.stdin, process.stdout, process.stderr],
    });
